...
- The challenge provides access to powerful Generative AI APIs, including:
  - **Phi3** for text generation
  - **SDXL** and **Flux** for image generation
  - **LLava-Next** for multimodal inputs
  - **Prediction Guard LLM / LVM APIs** - Let us know if you need API token to access the API provided by Prediction Guard.
  - **Pixel ML no code tool to prototype**
